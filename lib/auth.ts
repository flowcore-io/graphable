...
function extractUsableUserId(idToken: string | undefined): string | undefined {
	...
	try {
		...
		if (usableUserId) {
			...
			console.warn(
				"⚠️  usable_user_id claim not found in ID token - falling back to Keycloak user ID",
			);
			return undefined;
		}
	} catch (error) {
...
async function refreshAccessToken(token: JWT): Promise<JWT> {
	try {
		...
		if (!response.ok) {
			throw new Error(
				`Token refresh failed: ${refreshedTokens.error || response.statusText}`,
			);
		}
	...
	callbacks: {
		...
		async signIn({ user, account }) {
			...
			if (account?.id_token) {
				...
				if (!usableUserId) {
					console.warn(
						`❌ User ${user.email} attempted sign-in but missing usable_user_id claim in ID token`,
					);
					// Return false to reject sign-in - will redirect to error page
					return false;
				}
				console.log(
					`✅ User ${user.email} has usable_user_id: ${usableUserId}`,
				);
			} else {
				console.warn(
					`⚠️  User ${user.email} signed in but no ID token available`,
				);
		...
		async jwt({ token, user, account, profile }) {
			...
			if (account && user) {
				...
				if (usableUserId) {
					...
					try {
						...
						console.log(
							`✅ User record created/verified in database for ${usableUserId}`,
						);
					} catch (error) {
						// Don't block authentication if user creation fails
						// Log error but continue with authentication
						console.error(
							`⚠️  Failed to create user record for ${usableUserId}:`,
							error,
						);
					}
				}
			...
			if (!token.refreshToken) {
				console.log(
					"❌ No refresh token available - session migration required",
				);
				return {
					...token,
			...
			if (
				...
				typeof refreshedToken.idToken === "string" &&
				!refreshedToken.usableUserId
			) {
		...
		async session({ session, token }) {
			...
			if (token) {
				...
				if (!token.usableUserId) {
					console.error(
						`❌ Session invalidated for user ${token.email} - missing usableUserId (required for billing)`,
					);
				...
				if (token.error) {
					console.error(
						`⚠️  Session error: ${token.error} - user needs to re-login`,
					);
