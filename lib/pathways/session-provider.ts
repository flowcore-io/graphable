...
import {
	AUDIT_USER_ENTITY_TYPE,
	SessionPathwayBuilder,
...
export interface SessionPathwayContext {
	pathway: SessionPathwayBuilder<typeof pathways>;
	sessionId: string;
	userId: string;
	userType: "user" | "key";
}
...
export async function createSessionPathway(
	...
	const sessionPathway = new SessionPathwayBuilder(
		pathways,
		customSessionId,
...
export async function createSessionPathwayForAPI(): Promise<SessionPathwayContext | null> {
	try {
		...
		const sessionPathway = new SessionPathwayBuilder(pathways).withUserResolver(
			async () => ({
				entityId,
				entityType,
			}),
