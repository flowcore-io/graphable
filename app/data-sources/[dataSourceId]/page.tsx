...
export default function DataSourceViewPage() {
  ...
  useEffect(() => {
    if (isEditorCollapsed) return // Don't calculate height when collapsed

    const updateEditorHeight = () => {
      if (editorContainerRef.current) {
        const containerHeight = editorContainerRef.current.clientHeight
        // Reserve space for header (~60px), padding, and gap (~16px)
        // Use at least 50% of container or 300px minimum
        const availableHeight = containerHeight - 80
        const calculatedHeight = Math.max(300, Math.floor(availableHeight * 0.6))
    ...
    const timeoutId = setTimeout(updateEditorHeight, 200)
  ...
  const handleExecuteQuery = async (page: number = 1) => {
    ...
    // Auto-collapse editor after executing to show results
    setIsEditorCollapsed(true)

    setIsExecuting(true)
    setError(null)

    const queryToExecute = sqlQuery.trim()
    ...
    try {
      ...
      // Store the executed query so it's visible in results
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex gap-4 min-h-[calc(100vh-12rem)]">
          ...
          {/* Main Query Area */}
          <div className="flex-1 flex flex-col gap-4 min-w-0" ref={editorContainerRef}>
            {/* SQL Editor */}
            <Card className="flex flex-col shrink-0">
              ...
              {!isEditorCollapsed && (
                <CardContent className="p-0" style={{ height: `${editorHeight}px`, minHeight: "300px" }}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {executedQuery && (
                        <div className="mt-2 p-2 bg-muted rounded text-xs font-mono break-all">
                          <span className="text-muted-foreground">Executed query: </span>
                          <span className="text-foreground">{executedQuery}</span>
                        </div>
                      )}
                    </div>
