 ...
 */
export async function handlerTenantLinked(
	event: FlowcoreEvent<EventTenantLinked>,
) {
	...
	const existing = await db
		.select()
		.from(tenantLinks)
		.where(eq(tenantLinks.usableUserId, usableUserId))
		.limit(1);

	if (existing[0]) {
		console.log(
			`✅ Tenant link already exists for user ${usableUserId}, skipping`,
		);
		return;
	}
	...
	console.log(
		`✅ Tenant link created: workspace ${workspaceId} linked to user ${usableUserId}`,
	);
}
 ...
 */
export async function handlerTenantUnlinked(
	event: FlowcoreEvent<EventTenantUnlinked>,
) {
	...
	if (existing[0].workspaceId !== workspaceId) {
		console.log(
			`⚠️  Workspace mismatch: expected ${workspaceId}, found ${existing[0].workspaceId}, skipping`,
		);
		return;
	}
	...
	console.log(
		`✅ Tenant link deleted: workspace ${workspaceId} unlinked from user ${usableUserId}`,
	);
}
