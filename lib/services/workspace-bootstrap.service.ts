 ...
 */
async function ensureFragmentTypes(
	workspaceId: string,
	accessToken: string,
): Promise<void> {
	...
	const existingTypes = await usableApi.getFragmentTypes(
		workspaceId,
		accessToken,
	);
	const existingTypeNames = new Set(
		existingTypes.map((type) => type.name.toLowerCase()),
	);
	...
	for (const fragmentType of REQUIRED_FRAGMENT_TYPES) {
		if (existingTypeNames.has(fragmentType.name.toLowerCase())) {
			console.log(
				`Fragment type '${fragmentType.name}' already exists, skipping`,
			);
			continue;
		}
		...
		try {
			...
			console.error(
				`Failed to create fragment type '${fragmentType.name}':`,
				error,
			);
 ...
 */
async function createBaseConfigFragments(
	workspaceId: string,
	accessToken: string,
): Promise<void> {
