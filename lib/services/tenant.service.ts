...
import type { pathways } from "@/lib/pathways/pathways";
 ...
 */
export async function getTenantLink(
	usableUserId: string,
 ...
 */
export async function getTenantLinkByWorkspace(
	workspaceId: string,
): Promise<TenantLink | null> {
	const result = await db
		.select()
		.from(tenantLinks)
		.where(eq(tenantLinks.workspaceId, workspaceId))
		.limit(1);
...
export async function createTenantLink(
	usableUserId: string,
	workspaceId: string,
	sessionPathway: SessionPathwayBuilder<typeof pathways>,
): Promise<void> {
	...
	await sessionPathway.write("graphable.tenant.0/tenant.linked.0", {
		data: {
			workspaceId,
			usableUserId,
...
export async function deleteTenantLink(
	...
	await sessionPathway.write("graphable.tenant.0/tenant.unlinked.0", {
		data: {
